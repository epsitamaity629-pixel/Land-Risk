...
export default function LocationAnalysis() {
  ...
  const pastFloods = history.past_floods || history.floods || [];
  const pastLandslides = history.past_landslides || history.landslides || [];
  const pastLandrisks = history.past_landrisks || history.land_risks || [];
  ...
  const filteredHistory = historyFilter === "all" ? allHistory :
    historyFilter === "floods" ? pastFloods :
    historyFilter === "landslides" ? pastLandslides :
    pastLandrisks;
  ...
  const PRESET_CHIPS = [
    "Kolkata", "Durgapur", "Darjeeling", "Siliguri", "Shillong", "Gangtok", "Guwahati", "Kedarnath",
    "Itanagar", "Aizawl", "Kohima", "Imphal", "Agartala", "Wayanad", "Mumbai", "Pune", "Patna", "Ranchi", "Delhi", "Bengaluru"
  ];
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={forecast7d} margin={{ left: -10 }}>
                    ...
                    <Tooltip contentStyle={{ backgroundColor: "#0f172a", borderRadius: 8, color: "#fff", fontSize: 11 }}
                      formatter={(v, n) => [`${typeof v === 'number' ? v.toFixed(1) : v}/100`, n === "landslide_risk" ? "Landslide Risk" : "Flood Risk"]} />
              ...
              <div className="relative">
                ...
                <div className="space-y-3 max-h-72 overflow-y-auto pr-2">
                  {filteredHistory.map((ev, i) => {
                    const eventTitle = ev.type || ev.event_type || "Disaster Event";
                    const isFlood = eventTitle.toLowerCase().includes("flood") || (ev.type && ev.type.toLowerCase().includes("flood"));
                    const isLandslide = eventTitle.toLowerCase().includes("land") || eventTitle.toLowerCase().includes("slide") || eventTitle.toLowerCase().includes("rock") || (ev.type && (ev.type.toLowerCase().includes("land") || ev.type.toLowerCase().includes("slide")));
                    const typeIcon = isFlood ? "🌊" : isLandslide ? "⛰️" : "⚡";
                    const desc = ev.details || ev.impact || ev.description || ev.notes || `${ev.trigger_cause ?? "Rainfall-triggered"} event in ${ev.district ?? loc?.name}, ${ev.state ?? loc?.state}.`;
                    return (
                      <div key={i} className="flex items-start gap-4 pl-10 relative">
                        ...
                        <div className="flex-1 p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs">
                          <div className="flex items-center justify-between">
                            <span className="font-black text-slate-900">{ev.year ?? ev.event_date?.slice(0, 4)} — {eventTitle}</span>
                            {ev.severity && (
                          ...
                          <p className="text-slate-600 mt-0.5 leading-relaxed">{desc}</p>
                          {ev.casualties > 0 && <span className="text-red-600 font-bold text-[10px] mt-1 block">Reported casualties: {ev.casualties}</span>}
                          {ev.water_level && <span className="text-blue-600 font-medium text-[10px] block mt-0.5">Water Level: {ev.water_level}</span>}
                          {ev.trigger_mechanism && <span className="text-amber-700 font-medium text-[10px] block mt-0.5">Trigger: {ev.trigger_mechanism}</span>}
