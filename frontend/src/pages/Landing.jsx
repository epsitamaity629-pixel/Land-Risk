    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col justify-between relative">
      ...
      <header className="flex items-center justify-between px-6 md:px-12 py-4 bg-white border-b border-slate-200 shadow-xs sticky top-0 z-50">
        <div className="flex items-center gap-2.5">
          <div className="p-2 rounded-xl bg-emerald-700 text-white font-black shadow-xs">
          ...
          <div>
            ...
            <span className="text-[10px] text-emerald-700 font-bold block tracking-wider uppercase">
              Predict Risk · Warn Early · Protect Lives
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 bg-slate-100 border border-slate-200 rounded-lg p-0.5 text-xs">
            ...
            >
              ...
              <option value="ne">নেपाली (Nepali)</option>
            </select>
          </div>

          <Link
            to="/login"
            className="text-xs font-semibold text-slate-700 hover:text-slate-900 px-3 py-1.5 rounded-lg hover:bg-slate-100 transition hidden sm:inline"
          >
            Sign in
          </Link>
          <Link
      ...
      {/* Hero Section */}
      <section className="px-4 md:px-12 py-8 max-w-6xl mx-auto w-full space-y-8">
        <div className="text-center max-w-3xl mx-auto space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold tracking-wide uppercase">
            ...
            <span>BHU-SURAKSHA · MULTI-HAZARD INTELLIGENCE GRID</span>
          </div>

          <h1 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight leading-tight">
            Integrated Landslide & Flood Early Warning for{" "}
            <span className="text-emerald-700">NER & Pan-India Grid</span>
          </h1>

          <p className="text-slate-600 text-xs md:text-sm leading-relaxed max-w-2xl mx-auto font-medium">
            Fusing IoT pore-water inclinometers, IMD radar feeds, digital elevation models, and crowdsourced field observations into real-time 0–100 dual hazard risk predictions across 8 NER hill states and high-risk hotspots.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ["Monitored Stations", sum?.monitored_locations ?? "128", "Across NER & Pan-India"],
            ["Active Alerts", sum?.active_alerts ?? "4", "CAP Multi-Channel Trigger"],
            ["Regional Composite Risk", `${sum?.avg_risk_score ?? "58"} / 100`, "Multi-Hazard Index"],
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {[
            ...
            {
              icon: Siren,
              title: "Automated Early Warning & SMS Broadcast",
      ...
      <footer className="border-t border-slate-200 bg-white py-6 px-6 md:px-12 text-center text-xs text-slate-500">
        <p>© 2026 BHU-SURAKSHA Multi-Hazard Geo-Spatial Early Warning Platform. All rights reserved.</p>
        <p className="text-[10px] text-slate-400 mt-1">Built with IMD Meteorologic Radar, Open-Meteo API, Sentinel DEM & Machine Learning Geotechnical Models.</p>
      </footer>
