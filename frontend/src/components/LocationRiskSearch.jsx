...
export default function LocationRiskSearch({ onLocationSelected = null }) {
  ...
  const PAN_INDIA_CHIPS = [
    ...
    { name: "🏭 Durgapur (WB)", q: "Durgapur" },
    { name: "🌊 Mumbai (MH)", q: "Mumbai" },
    { name: "🚗 Pune (MH)", q: "Pune" },
    { name: "🏛️ Patna (Bihar)", q: "Patna" },
    { name: "🏞️ Ranchi (JH)", q: "Ranchi" },
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-slate-100 gap-3">
              ...
              {(() => {
                const pastFloods = categorizedHistory.past_floods || categorizedHistory.floods || [];
                const pastLandslides = categorizedHistory.past_landslides || categorizedHistory.landslides || [];
                const pastLandrisks = categorizedHistory.past_landrisks || categorizedHistory.land_risks || [];
                const totalEvents = categorizedHistory.total_historical_events || (pastFloods.length + pastLandslides.length + pastLandrisks.length);
                return (
                  <div className="flex items-center gap-1.5 p-1 bg-slate-100 border border-slate-200 rounded-xl">
                    ...
                    >
                      🌊 {t.previousFloods || "Floods"} ({pastFloods.length})
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      🏔️ {t.previousLandslides || "Landslides"} ({pastLandslides.length})
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      🚜 {t.previousLandrisks || "Land Risk"} ({pastLandrisks.length})
                    </button>
                  </div>
                );
              })()}
                  ...
                  {(historyCategory === "all" || historyCategory === "floods") && (
                    <div className="p-4 rounded-xl bg-blue-50/40 border border-blue-200 space-y-2.5">
                      <div className="flex items-center justify-between font-extrabold text-blue-900 text-xs">
                        ...
                        <span className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 text-[10px]">{pastFloods.length} Events</span>
                      </div>

                      {pastFloods.length > 0 ? (
                        <div className="space-y-2">
                          {pastFloods.map((item, idx) => (
                            <div key={idx} className="p-3 bg-white rounded-lg border border-blue-100 shadow-xs text-xs space-y-1">
                              <div className="flex items-center justify-between font-bold">
                                <span className="text-slate-900 font-black">{item.year} · {item.type || item.event_type}</span>
                              ...
                              <p className="text-slate-600 text-[11px]">{item.details || item.impact || item.description || item.notes}</p>
                              <div className="flex justify-between text-[10px] text-slate-500 pt-1 border-t border-slate-100">
                                <span>Water Level: <strong>{item.water_level || "Above Danger Mark"}</strong></span>
                                <span className="text-red-600 font-bold">Casualties: {item.casualties || 0}</span>
                              </div>
                            </div>
                          ))}
                        </div>
                  ...
                  {(historyCategory === "all" || historyCategory === "landslides") && (
                    <div className="p-4 rounded-xl bg-amber-50/40 border border-amber-200 space-y-2.5">
                      <div className="flex items-center justify-between font-extrabold text-amber-900 text-xs">
                        ...
                        <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 text-[10px]">{pastLandslides.length} Events</span>
                      </div>

                      {pastLandslides.length > 0 ? (
                        <div className="space-y-2">
                          {pastLandslides.map((item, idx) => (
                  ...
                  {(historyCategory === "all" || historyCategory === "landrisks") && (
                    <div className="p-4 rounded-xl bg-teal-50/40 border border-teal-200 space-y-2.5">
                      <div className="flex items-center justify-between font-extrabold text-teal-900 text-xs">
                        ...
                        <span className="px-2 py-0.5 rounded-full bg-teal-100 text-teal-800 text-[10px]">{pastLandrisks.length} Events</span>
                      </div>

                      {pastLandrisks.length > 0 ? (
                        <div className="space-y-2">
                          {pastLandrisks.map((item, idx) => (
                        ...
                        <p className="text-xs text-slate-500 italic py-2">No ground subsidence or bank erosion events logged.</p>
                      )}
                    </div>
                  )}
                </div>
